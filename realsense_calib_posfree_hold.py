...
import utils1.pcd_utils as pcdu
...
def phoxi_computeeeinphx(motion_planner_x, realsense_client, grasp, objcm, objpos, objrot, objrelpos, objrelrot,
                         criteriaradius=None):
    """
    ...
    for axisid in range(3):
        ...
        for angle in rangeaxis[axisid][1]:
            ...
            if success:
                phxpos = realsense_client.getcenter()
                print("phxpos:", phxpos)
...
def phoxi_computeeeinphx_loadpath(motion_planner_x: object, realsense_client: object, criteriaradius: object = None) -> object:
    def fitfunc(p, coords):
        x0, y0, z0, R = p
        x, y, z = coords.T
        return np.sqrt((x - x0) ** 2 + (y - y0) ** 2 + (z - z0) ** 2)
...
def phoxi_computeboardcenterinhand(motion_planner_x, realsense_client, grasp, objcm, objpos, objrot, objrelpos, objrelrot,
                                   criteriaradius=None):
    # eeposinphx, bcradius = phoxi_computeeeinphx(motion_planner_x, phxi_client, grasp, objcm, objpos, objrot,
    #                                             objrelpos, objrelrot)
    eeposinphx, bcradius = phoxi_computeeeinphx_loadpath(motion_planner_x, realsense_client)

    # moveback
    success = motion_planner_x.goto_objmat4_goal_x(grasp, objrelpos, objrelrot, rm.homobuild(objpos, objrot), objcm)
    if success:
        hcinphx = realsense_client.getcenter()
    print(hcinphx)
    ...
    if success:
        hxinphx = realsense_client.getcenter()
    print(hxinphx)
    ...
    if success:
        hyinphx = realsense_client.getcenter()
    print(hyinphx)
    ...
    if success:
        hzinphx = realsense_client.getcenter()
    print(hzinphx)
...
def phoxi_calib_auto(motion_planner_x, realsense_client, grasp, objcm, objrelpos, objrelrot, relpos, dump_id=''):
    realposlist = []
    phxposlist = []
...
def show_path(mp, f_name, objcm, objrelpos, objrelrot):
    ...
    with open(os.path.join(config.ROOT, "camcalib/data/phoxi_realpos.pkl"), 'rb') as f:
        realposlist = pickle.loads(f.read())
...
if __name__ == '__main__':
    ...
    relpos = phoxi_computeboardcenterinhand(mp_x_lft, realsense_client, grasp, objcm, objpos, objrot, objrelpos,
                                            objrelrot)
    print("relpos:", relpos)
    amat = phoxi_calib_auto(mp_x_lft, realsense_client, grasp, objcm, objrelpos, objrelrot, -relpos, dump_id=dump_id)
    ...
    pcd = realsense_client.rgbd2pcd()
    grayimg = realsense_client.get_gray()

    pandag = pg.PandaDecorator()

    print(amat)
    offset_rot = rm.rotmat_from_euler(0, 0, 0)
    offset_trans = np.asarray([0, 0, 0])
    offset_homo = rm.homobuild(offset_trans, offset_rot)
    amat = np.dot(offset_homo, amat)
    # amat[:3, 3] = amat[:3, 3] + np.asarray([200, -50, 35])
    print("final", amat)
    pandag.plotAxis(base.render, amat[:3, 3], amat[:3, :3])
    realpcd = pcdu.trans_pcd(pcd, amat)
    # pg.packpandageompnts(realpcd).reparentTo(base.render)
