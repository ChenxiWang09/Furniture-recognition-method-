...
if __name__ == "__main__":
    ...
    for ex_times in range(test_time):
        ...
        # --------------------------------
        # attribute: shape(contour) by using fdcm
        # --------------------------------
        for i in range(n):
            j = i + 1
            ...
            im_dst = cv2.imread(
                "C:/Users/wangq/PycharmProjects/pythonProject/intelrealsense_wcx/templateimage/smallsize/" + str(
                    j) + ".png")
            ...
            fdcmstarttime = time.time()
            partnumber, value = eng.chamfermatching_nopic_forpython(sav, j, size_min_x, size_min_y, size_max_x,
                                                                    size_max_y, times, nargout=2)
        ...
        for i in range(4):
            ...
            for j in range(-boxsize, 1 + boxsize):
                for k in range(-boxsize, 1 + boxsize):
                    dis, camera_coordinate_matrix = get_3d_camera_coordinate(
                        [int(depth_pixel[0]) + j, int(depth_pixel[1]) + k],
        ...
        for i in range(n):
            ...
            print("partnumber: %d" % j, "  sizecost: %.4f" % sizecost[i], "  shapecost: %.4f" % shapecost[i],
                  "  cost: %.4f" % cost)
        ...
        ws.cell(row=ex_times + 1 + m, column=13, value=str((shapecost[7] + sizecost[7])))
        ws.cell(row=ex_times + 1 + m, column=14, value=str(endtime - starttime))
        ws.cell(row=ex_times + 1 + m, column=15, value=str((cannytime-starttime)))
        ws.cell(row=ex_times + 1 + m, column=16, value=str((matchingstarttime - cannytime)))
        ws.cell(row=ex_times + 1 + m, column=17, value=str((sizeendtime - matchingstarttime)))
        ws.cell(row=ex_times + 1 + m, column=18, value=str((endtime - sizeendtime)))
